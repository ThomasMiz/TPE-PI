...
{
    specieCounterADT s;
    if (!tryCalloc((void **)&s, 1, sizeof(struct specieCounterCDT)))
        return NULL;
    s->counter.previousCharIndex = -1;
    return s;
}
...
void freeSpecieCounter(specieCounterADT counter)
{
    ...
    for (int i = 0; i < NODE_COUNT; i++)
        if (counter->counter.nextNodes[i] != NULL)
            freeTrieRecursive(counter->counter.nextNodes[i]);
...
enum SPECIE_ERR specieAddOne(specieCounterADT counter, const char *specieName, size_t len)
{
...
char *specieMostPopular(specieCounterADT counter)
{
    TNode *mostPopular = NULL;
    size_t mostDepth = 0;

    // We recursively search for the node with the highest node.count.
    findMostPopularRecursive(&(counter->counter), LOWEST_NODE_DEPTH, &mostPopular, &mostDepth);
    ...
    // Since the counter doesn't keep track of casing, we uppercase the first character.
    str[0] = toupper(str[0]);
